...
int main(int argc, char **argv) {
    ...
    std::cout << "This rank has "
              << (xBounds[myXRank] - xBounds[myXRank + 1]) * (yBounds[myXRank] - yBounds[myXRank + 1]) << " blocks \n";
    /***************
     * INIT OUTPUT *
     ***************/
    ...
    MPI_Win_create(writeBlock.h.getRawPointer(), (nxRequested + 2) * (nyRequested * 2), sizeof(float), MPI_INFO_NULL,
                   MPI_COMM_WORLD, &writeBlockWin_h);
    MPI_Win_create(writeBlock.hu.getRawPointer(), (nxRequested + 2) * (nyRequested * 2), sizeof(float), MPI_INFO_NULL,
                   MPI_COMM_WORLD, &writeBlockWin_hu);
    MPI_Win_create(writeBlock.hv.getRawPointer(), (nxRequested + 2) * (nyRequested * 2), sizeof(float), MPI_INFO_NULL,
                   MPI_COMM_WORLD, &writeBlockWin_hv);
...
#pragma omp parallel
    ...
    for (int i = 0; i < numberOfCheckPoints; i++) {
        ...
        int xLower = xBounds[myXRank];
        int xUpper = xBounds[myXRank + 1];
        int yLower = yBounds[myYRank];
        int yUpper = yBounds[myYRank + 1];
            ...
            for (int x = xBounds[myXRank]; x < xBounds[myXRank + 1]; x++) {
                for (int y = yBounds[myYRank]; y < yBounds[myYRank + 1]; y++) {
                    ...
                    for (int j = 1; j < blocks[x][y]->nx + 1; j++) {
                        int x_pos = x * x_blocksize;
                        int y_pos = y * y_blocksize;
                        MPI_Put(blocks[x][y]->h.getRawPointer() + 1 + (blocks[x][y]->ny + 2) * j, blocks[x][y]->ny,
                                MPI_FLOAT,
                                0, 1 + (nyRequested + 2) * (1 + j + x_pos) + y_pos, blocks[x][y]->ny, MPI_FLOAT,
                                writeBlockWin_h);
                        MPI_Put(blocks[x][y]->hu.getRawPointer() + 1 + (blocks[x][y]->ny + 2) * j, blocks[x][y]->ny,
                                MPI_FLOAT,
                                0, 1 + (nyRequested + 2) * (1 + j + x_pos) + y_pos, blocks[x][y]->ny, MPI_FLOAT,
                                writeBlockWin_hu);
                        MPI_Put(blocks[x][y]->hv.getRawPointer() + 1 + (blocks[x][y]->ny + 2) * j, blocks[x][y]->ny,
                                MPI_FLOAT,
                                0, 1 + (nyRequested + 2) * (1 + j + x_pos) + y_pos, blocks[x][y]->ny, MPI_FLOAT,
                                writeBlockWin_hv);
                    }
