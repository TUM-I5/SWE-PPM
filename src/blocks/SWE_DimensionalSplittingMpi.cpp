...
void SWE_DimensionalSplittingMpi::setGhostLayer() {
    ...
    //::cout << "Send timestep " << totalLocalTimestep << std::endl;
	if (boundaryType[BND_LEFT] == CONNECT && isSendable(BND_LEFT)) {
		int startIndex = ny + 2 + 1;

        MPI_Isend(&totalLocalTimestep, 1, MPI_FLOAT, neighbourRankId[BND_LEFT], MPI_TAG_TIMESTEP, MPI_COMM_WORLD, &req);
        MPI_Request_free(&req);
	...
	if (boundaryType[BND_RIGHT] == CONNECT && isSendable(BND_RIGHT)) {
		int startIndex = nx * (ny + 2) + 1;
        MPI_Isend(&totalLocalTimestep, 1, MPI_FLOAT, neighbourRankId[BND_RIGHT], MPI_TAG_TIMESTEP, MPI_COMM_WORLD, &req);
        MPI_Request_free(&req);
	...
	if (boundaryType[BND_BOTTOM] == CONNECT && isSendable(BND_BOTTOM)) {
        MPI_Isend(&totalLocalTimestep, 1, MPI_FLOAT, neighbourRankId[BND_BOTTOM], MPI_TAG_TIMESTEP, MPI_COMM_WORLD, &req);
        MPI_Request_free(&req);
	...
	if (boundaryType[BND_TOP] == CONNECT && isSendable(BND_TOP)) {
        MPI_Isend(&totalLocalTimestep, 1, MPI_FLOAT, neighbourRankId[BND_TOP], MPI_TAG_TIMESTEP, MPI_COMM_WORLD, &req);
        MPI_Request_free(&req);
	...
	if (boundaryType[BND_LEFT] == CONNECT && isReceivable(BND_LEFT)) {
        ...
        MPI_Irecv(&borderTimestep[BND_LEFT], 1, MPI_FLOAT, neighbourRankId[BND_LEFT], MPI_TAG_TIMESTEP, MPI_COMM_WORLD, &recvReqs[3] );
	} else {
	...
	if (boundaryType[BND_RIGHT] == CONNECT && isReceivable(BND_RIGHT)) {
        ...
        MPI_Irecv(&borderTimestep[BND_RIGHT], 1, MPI_FLOAT, neighbourRankId[BND_RIGHT], MPI_TAG_TIMESTEP, MPI_COMM_WORLD, &recvReqs[7] );
	} else {
	...
	if (boundaryType[BND_BOTTOM] == CONNECT && isReceivable(BND_BOTTOM)) {
        ...
        MPI_Irecv(&borderTimestep[BND_BOTTOM], 1, MPI_FLOAT, neighbourRankId[BND_BOTTOM], MPI_TAG_TIMESTEP, MPI_COMM_WORLD, &recvReqs[11] );
	...
	if (boundaryType[BND_TOP] == CONNECT && isReceivable(BND_TOP)) {
        ...
        MPI_Irecv(&borderTimestep[BND_TOP], 1, MPI_FLOAT, neighbourRankId[BND_TOP], MPI_TAG_TIMESTEP, MPI_COMM_WORLD, &recvReqs[15] );
