...
void SWE_DimensionalSplittingChameleon::setGhostLayer() {
	...
	if (boundaryType[BND_LEFT] == CONNECT) {
        ...
        MPI_Isend(getTotalLocalTimestep(), 1, MPI_FLOAT, neighbourRankId[BND_LEFT], ((int)originY)&tagTimestep, MPI_COMM_WORLD, &req);
	...
	if (boundaryType[BND_RIGHT] == CONNECT) {
        ...
        MPI_Isend(getTotalLocalTimestep(), 1, MPI_FLOAT, neighbourRankId[BND_RIGHT], ((int)originY)&tagTimestep, MPI_COMM_WORLD, &req);
        MPI_Request_free(&req);
	...
	if (boundaryType[BND_BOTTOM] == CONNECT) {
        MPI_Isend(getTotalLocalTimestep(), 1, MPI_FLOAT, neighbourRankId[BND_BOTTOM], ((int)originY)&tagTimestep, MPI_COMM_WORLD, &req);
        MPI_Request_free(&req);
		//int code = 
	...
	if (boundaryType[BND_TOP] == CONNECT) {
        MPI_Isend(getTotalLocalTimestep(), 1, MPI_FLOAT, neighbourRankId[BND_TOP], ((int)originY)&tagTimestep, MPI_COMM_WORLD, &req);
        MPI_Request_free(&req);
...
void SWE_DimensionalSplittingChameleon::receiveGhostLayer() {
	...
	if(code != MPI_SUCCESS)
		printf("%d: No success %d\n", myRank, code);
	//@todo add check for timestep size received
	//@todo receive again if border ts smaller than local ts
	//@todo if local smaller than border than perform interpolation and continue computation till at desired max ts
    //@todo maybe migrate if_statement below to SWE_BLOCK
	if(localTimestepping){
...
void computeNumericalFluxesHorizontalKernel(SWE_DimensionalSplittingChameleon* block, float* maxTimestep, float* h_data, float* hu_data, float* b_data,
	...
	if(block->localTimestepping){
        *maxTimestep = block->getRoundedTimestep(block->maxTimestep);
	}else {
...
void SWE_DimensionalSplittingChameleon::computeNumericalFluxesHorizontal() {
    ...
    if(!receivedAllGhostlayers()) return;
