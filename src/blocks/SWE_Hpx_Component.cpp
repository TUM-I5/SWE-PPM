...
#include "scenarios/SWE_simple_scenarios.hh"
 ...
 namespace remote
    {
        client::SWE_DimensionalSplittingComponent newComponent (int myHpxRank, int totalHpxRanks, float simulationDuration, int numberOfCheckPoints,
                int nxLocal, int nyLocal, float dxSimulation, float dySimulation, float  localOriginX,
                float localOriginY,
                std::array<BoundaryType,4> boundaries,std::array<int,4> myNeighbours) {
            return hpx::new_<client::SWE_DimensionalSplittingComponent>(hpx::find_here(),myHpxRank,totalHpxRanks,simulationDuration,numberOfCheckPoints,
                                                                        nxLocal,nyLocal,dxSimulation,dySimulation,localOriginX,localOriginY,boundaries, myNeighbours);
        }
     client::SWE_DimensionalSplittingComponent newComponentAsagi (int myHpxRank, int totalHpxRanks, float simulationDuration, int numberOfCheckPoints,
                                                             int nxLocal, int nyLocal, float dxSimulation, float dySimulation, float  localOriginX,
                                                             float localOriginY,
                                                             std::array<BoundaryType,4> boundaries,std::array<int,4> myNeighbours, std::string const &batFile,
    ...
    SWE_Hpx_Component::SWE_Hpx_Component(int totalRanks,float simulationDuration,
...
*/
            ...
            blocksFut.push_back(hpx::async(newComponent,myHpxRank,totalHpxRanks,simulationDuration,numberOfCheckPoints,
                                           nxLocal,nyLocal,dxSimulation,dySimulation,localOriginX,localOriginY,boundaries, myNeighbours));
           //simulationBlocks.push_back(func());
#endif
                ...
                hpx::dataflow(hpx::util::unwrapping([this](std::vector<client::SWE_DimensionalSplittingComponent> blocks) -> void {
                  ...
                  //  for(auto &block : blocks)simulationBlocks.push_back(block);
