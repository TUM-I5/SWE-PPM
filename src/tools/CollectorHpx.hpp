...
public:
    ...
    void collect ()  {
        ...
        result_ctrs[CTR_EXCHANGE] = hpx::all_reduce("exchange_reduce",total_ctrs[CTR_EXCHANGE].count(),std::plus<double>{}).get();
        result_ctrs[CTR_REDUCE] = hpx::all_reduce("reduction_reduce",total_ctrs[CTR_REDUCE].count(),std::plus<double>{}).get();
