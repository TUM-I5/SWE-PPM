    ...
    static CollectorCharm deserialize(double *arr,bool all = false) {
        ...
        if(all){
            for (int i = 0; i < 4; i++) {
                ...
                if(i==CTR_WALL)
                    std::cout<<  "recv measure_ctr "<< std::chrono::time_point<std::chrono::steady_clock>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::duration<double>(arr[i+5]))).time_since_epoch().count() << std::endl;
