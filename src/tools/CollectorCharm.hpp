    ...
    double *serialize(double *arr, bool all = false) {
        ...
        if(all){
            for (int i = 0; i < 4; i++) {
                ...
                arr[i + 5] =(double)(std::chrono::time_point_cast<std::chrono::milliseconds>(measure_ctrs[i]).time_since_epoch().count());
            }
        }
    ...
    static CollectorCharm deserialize(double *arr,bool all = false) {
        ...
        if(all){
            for (int i = 0; i < 4; i++) {
                ...
                if(i==CTR_WALL)
                    std::cout<<  "recv measure_ctr "<< std::chrono::time_point<std::chrono::steady_clock>(std::chrono::duration_cast<std::chrono::milliseconds>((long)arr[i+5])).time_since_epoch().count() << std::endl;
